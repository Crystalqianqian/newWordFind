...
public class Reduce_SplitWord_First extends Reducer<Text, Text, NullWritable, Text> {
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		//-1表示返回所有的子字符串数
		String[] keyArray = key.toString().split("\t", -1);
		if ("TERM".equals(keyArray[0])) {
			int number = 0;
			...
			for (Text text : values) {
				...
				if (rightMap.get(valueArray[3]) == null) {
					rightMap.put(valueArray[3], 1);
				} else {
					int rightNumber = rightMap.get(valueArray[3]) + 1;
					rightMap.put(valueArray[3], rightNumber);
				}
			}
			double leftFreq = 0;
			double rightFreq = 0;
			...
			context.write(NullWritable.get(), new Text("SIZE" + "\t" +"总数"+"\t"+ size ));
		}
	}
}
